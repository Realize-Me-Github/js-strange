...
describe("Range", function () {
	...
	describe(".prototype.isEmpty", function () {
		...
		it("must return false if exclusive with non-equivalent endpoints", function () {
	...
	describe(".prototype.isBounded", function () {
		...
		it("must return false given a left unbounded range with Infinity", function () {
			new Range(-Infinity, 1).isBounded().must.be.false();
		});

		it("must return false given a right unbounded range with Infinity", function () {
	...
	describe(".prototype.isUnbounded", function () {
		...
		it("must return true given a left unbounded range with Infinity", function () {
			new Range(-Infinity, 1).isUnbounded().must.be.true();
		});

		it("must return true given a right unbounded range with Infinity", function () {
	...
	describe(".prototype.contains", function () {
		...
		it("must return true if one endpoint unbounded and on inclusive boundary", function () {
			new Range(0, null).contains(0).must.be.true();
			new Range(null, 0).contains(0).must.be.true();
		});

		it("must return false if one endpoint unbounded and on exclusive boundary", function () {
	...
	describe(".prototype.intersects", function () {
		...
		function testWithBounds(bounds) {
			...
			it("must return true when one encloses the other and is unbounded", function () {
				var a = new Range(0, 10, bounds);
		...
		it("must return true when one encloses the other, but one exclusive", function () {
		...
		it("must return false when consecutive, but both exclusive", function () {
			var a = new Range(0, 10, "[)");
			var b = new Range(10, 20, "(]");
			intersects(a, b).must.be.false();
		});
	...
	describe(".compareBeginToEnd", function () {
		...
		describe("with different bounds", function () {
			...
			it("must return -1 if less", function () {
				compare(new Range(-1, 10, "[]"), new Range(-10, 0, "()")).must.equal(
					-1,
				);
				compare(new Range(-1, 10, "()"), new Range(-10, 0, "[]")).must.equal(
					-1,
	...
	describe(".parse", function () {
		...
		it("must parse string with infinite bounds given parse function", function () {
	...
	describe(".union", function () {
		...
		describe("with exclusive bounds", function () {
			...
			it("must return a union given two close, but non-consecutive ranges", function () {
				var a = new Range(0, 10, "()");
				var b = new Range(10, 20, "()");
				union(a, b).must.eql(new Range(0, 20, "()"));
			});
