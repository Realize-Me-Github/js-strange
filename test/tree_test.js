...
describe("RangeTree", function () {
	describe(".prototype.search", function () {
		describe("given value", function () {
			...
			it("must return an empty array if with exclusive left boundary", function () {
				var tree = RangeTree.from([new Range(0, 10, "(]")]);
				tree.search(0).must.be.empty();
			});

			it("must return an empty array if with exclusive right boundary", function () {
				var tree = RangeTree.from([new Range(0, 10, "[)")]);
			...
			it("must return multiple ranges ordered by ascending end", function () {
				...
				tree
					.search(5)
					.must.eql([new Range(0, 5), new Range(0, 10), new Range(0, 15)]);
			});
		...
		describe("given range", function () {
			...
			it("must return multiple ranges if within 3", function () {
				...
				tree
					.search(new Range(32, 37))
					.must.eql([new Range(15, 35), new Range(30, 40)]);
			});
			...
			it("must return multiple ranges with equal begin", function () {
				...
				tree
					.search(new Range(30, 32))
					.must.eql([new Range(15, 35), new Range(15, 40)]);
			});
			...
			it("must return multiple ranges if on two sides", function () {
				...
				tree
					.search(new Range(6, 13))
					.must.eql([new Range(0, 15), new Range(10, 25)]);
			});
			...
			it("must return multiple ranges if all equal", function () {
				...
				tree
					.search(new Range(7, 13))
					.must.eql([new Range(5, 15), new Range(5, 15)]);
			});
			...
			it("must return multiple ranges if one equal but exclusive", function () {
				...
				tree
					.search(new Range(10, 10))
					.must.eql([new Range(5, 15), new Range(10, 20)]);
			});
			...
			it("must return range farther than later ranges in original bug report", function () {
				var tree = RangeTree.from([
					new Range(10, 20, "[)"),
					new Range(20, 30, "[)"),
					new Range(30, 40, "[)"),
